...
============================================================
PULMO GUIDE
DAY 4 — COMPREHENSIVE FINAL EVALUATION
============================================================
Deterministic. No LLM. No LangChain.
Safe with missing files.
Flexible JSON parsing tuned to the project's evaluation artifacts.

Final reported retrieval metrics:
    Precision@5
    Recall@5
    Hit Rate@5
    MRR@5

Final reported latency metrics:
    Average
    Median
    Min
    Max

System Architecture is NOT included in the automated report.
Hackathon weighting percentages are NOT displayed in the report.
============================================================
"""
...
def normalize_metric(value: Optional[float]) -> Optional[float]:
    ...
    Already-percentage values remain unchanged.
    """
    if value is None:
        return None
...
def load_all_files() -> Dict[str, Any]:
    ...
    for name, path in FILES.items():
        ...
        if data is None:
            print(
                f"[MISSING] {name:<24} {path.name}"
            )

        else:
            print(
                f"[OK]      {name:<24} {path.name}"
            )
...
def evaluate_claim_verification(
    ...
    return {
        ...
        "claim_coverage_rate": (
            percentage(
                supported_claims + partially_supported,
                total_claims,
            )
            if total_claims > 0
            else None
...
def evaluate_faithfulness(
    ...
    return {
        ...
        "strict_rate": percentage(
            faithful,
            total_tests,
        ),
        "broad_rate": percentage(
            faithful + partially,
            total_tests,
        ),
    }
...
#   MRR@5
#
# NDCG@5 and MAP@5 intentionally removed.
# ============================================================
...
def extract_retrieval_metric_dict(
    values: Any
) -> Dict[str, float]:
...
def evaluate_retrieval(
    ...
    test_cases = (
        safe_int(config.get("num_questions"))
        or len(per_question)
    )
    ...
    if (
        ...
        for name, vals in averages.items():
            ...
            metrics = extract_retrieval_metric_dict(
                vals
            )
...
def calculate_retrieval_quality(
    ...
    if (
        not retrieval.get("available")
        or retrieval.get("retrieval_score") is None
    ):
...
def evaluate_refusal_threshold(
    ...
    best = max(
        threshold_results,
        key=lambda t:
        safe_float(t.get("accuracy"))
        if safe_float(t.get("accuracy"))
        is not None
        else -1.0,
    )
    ...
    return {
        ...
        "accuracy": normalize_metric(
            safe_float(best.get("accuracy"))
        ),
        "precision": normalize_metric(
            safe_float(best.get("precision"))
        ),
        "recall": normalize_metric(
            safe_float(best.get("recall"))
        ),
        "correct_accept": best.get(
            "correct_accept"
        ),
        "correct_refuse": best.get(
            "correct_refuse"
        ),
        "false_accept": best.get(
            "false_accept"
        ),
        "false_reject": best.get(
            "false_reject"
        ),
        "thresholds_tested": len(
            threshold_results
        ),
    }


def evaluate_refusal_ground_truth_only(
    data: Optional[Any]
) -> Dict[str, Any]:
...
def choose_refusal_metrics(
    loaded: Dict[str, Any]
) -> Dict[str, Any]:

    threshold_data = loaded.get(
        "refusal_threshold"
    )

    if threshold_data is not None:

        metrics = evaluate_refusal_threshold(
            threshold_data
        )
    ...
    return evaluate_refusal_ground_truth_only(
        loaded.get("refusal_evaluation")
    )
...
def evaluate_threshold_experiment(
    data: Optional[Any]
) -> Dict[str, Any]:
    ...
    return {
        ...
        "accept_precision": normalize_metric(
            safe_float(
                best.get("accept_precision")
            )
        ),
        "relevant_recall": normalize_metric(
            safe_float(
                best.get("relevant_recall")
            )
        ),
        "acceptance_rate": normalize_metric(
            safe_float(
                best.get("acceptance_rate")
            )
        ),
        "thresholds_tested": len(
            threshold_results
...
def calculate_grounding_score(
    ...
    claim_value = claim_metrics.get(
        "claim_support_rate"
    )

    if (
        claim_metrics.get("available")
    ...
    citation_value = citation_metrics.get(
        "citation_accuracy"
    )

    if (
        citation_metrics.get("available")
        and citation_value is not None
    ):
        components.append(
            (citation_value, 0.30)
        )

    faithfulness_value = faithfulness_metrics.get(
        "strict_rate"
    )

    if (
        faithfulness_metrics.get("available")
        and faithfulness_value is not None
    ):
        components.append(
            (faithfulness_value, 0.25)
        )

    confidence_value = confidence_metrics.get(
        "accuracy"
    )

    if (
        confidence_metrics.get("available")
...
def calculate_safety_score(
    ...
    safety_value = safety.get(
        "accuracy"
    )

    if (
        safety.get("available")
    ...
    refusal_value = refusal.get(
        "accuracy"
    )

    if (
        refusal.get("available")
...
#
# Architecture is intentionally not represented here.
# ============================================================
...
def calculate_measurable_score(
    ...
    retrieval_value = retrieval_quality.get(
        "score"
    )

    if (
        retrieval_quality.get("available")
        and retrieval_value is not None
    ):
        components.append(
            (retrieval_value, 30)
        )

    grounding_value = grounding_score.get(
        "percentage"
    )

    if (
        grounding_score.get("available")
        and grounding_value is not None
    ):
        components.append(
            (grounding_value, 25)
        )

    evaluation_value = dataset_depth.get(
    ...
    safety_value = safety_score.get(
        "score"
    )

    if (
        safety_score.get("available")
        and safety_value is not None
    ):
        components.append(
            (safety_value, 15)
    ...
    return {
        ...
        "note": (
            "Aggregated measurable evaluation "
            "indicator based on available quantitative evidence."
        ),
    }
...
#
# IMPORTANT:
# No hackathon percentages are displayed in the report.
# No System Architecture section.
# ============================================================
...
def build_rubric_summary(
    ...
    return {
        ...
        "retrieval_quality": {
            "measured_score": retrieval_quality.get(
                "score"
            ),
            "status": (
                "MEASURED"
                if retrieval_quality.get("available")
                else "NOT_AVAILABLE"
            ),
            "selected_configuration": retrieval_quality.get(
                "selected_config"
            ),
            "components": [
                "Semantic retrieval",
        ...
        "grounding_citation": {
            "measured_score": grounding_score.get(
                "percentage"
            ),
            "status": (
                "MEASURED"
                if grounding_score.get("available")
                else "NOT_AVAILABLE"
            ),
            "components": [
                "Claim support",
        ...
        "evaluation_depth": {
            "measured_score": dataset_depth.get(
                "completion_percentage"
            ),
            "status": (
                "MEETS_TARGET"
                if dataset_depth.get(
                    "meets_20_question_target"
                )
                else "BELOW_20_CASES"
            ),
            "test_cases": dataset_depth.get(
                "test_cases",
                0,
            ),
            "recommended_minimum": dataset_depth.get(
                "recommended_minimum",
                20,
            ),
        },
        ...
        "safety_ux": {
            "measured_score": safety_score.get(
                "score"
            ),
            "status": (
                "MEASURED"
                if safety_score.get("available")
                else "NOT_AVAILABLE"
            ),
            "components": [
                "Safety tests",
...
def analyze_faithfulness_failures(
    data: Optional[Any]
) -> List[Dict[str, Any]]:
...
def build_report(
    ...
    lines.append("=" * 78)
    lines.append(
        "PULMO GUIDE — COMPREHENSIVE FINAL EVALUATION"
    )
    lines.append("=" * 78)
    lines.append("")
    lines.append(
        "Deterministic evaluation — No LLM required"
    )
    lines.append("")
    ...
    lines.append(
        "Measurable Project Indicator: "
        + fmt(
            measurable_score.get(
                "score"
            )
        )
        + "%"
    ...
    if retrieval_metrics["available"]:
        ...
        lines.append(
            "Retrieval evaluation not available."
        )
    ...
    if hybrid_metrics["available"]:
        ...
        lines.append(
            "  Not included in the main retrieval score."
        )
    ...
    if refusal_metrics.get(
        ...
        lines.append(
            ...
            "no actual system decisions to score."
        )
    ...
    if threshold_experiment_metrics.get(
        "available"
    ):
        ...
        lines.append(
            "  Test cases: "
            + str(
                threshold_experiment_metrics.get(
                    "test_cases"
                )
            )
        )
        ...
        if threshold_experiment_metrics.get(
            ...
            lines.append(
                "  Note: all tested thresholds "
                "produced identical accept/reject outcomes."
            )
    ...
    lines.append(
        "10. MEASURABLE PROJECT INDICATOR"
    )
    ...
    lines.append(
        "11. EVALUATION ARTIFACT AVAILABILITY"
    )
    ...
    lines.append("=" * 78)
    lines.append(
        "END OF COMPREHENSIVE FINAL EVALUATION"
    )
    lines.append("=" * 78)
...
def main():
    ...
    print(
        "Evaluation is separated from inference."
    )
    ...
    claim_metrics = evaluate_claim_verification(
        loaded["claim_verification"]
    )
    ...
    confidence_metrics = evaluate_confidence_gate(
        loaded["confidence_gate"]
    )
    ...
    day3_test_metrics = evaluate_day3_test_suite(
        loaded["day3_test"]
    )

    refusal_metrics = choose_refusal_metrics(
        loaded
    )

    threshold_experiment_metrics = (
        evaluate_threshold_experiment(
            loaded["threshold_experiment"]
        )
    )

    latency_metrics = evaluate_latency(
        loaded["latency"]
    )

    claim_extraction_metrics = (
        evaluate_claim_extraction(
            loaded["claim_extraction"]
        )
    )

    dataset_depth = evaluate_dataset_depth(
    ...
    claim_failures = analyze_claim_failures(
        loaded["claim_verification"]
    )

    faithfulness_failures = (
        analyze_faithfulness_failures(
            loaded["faithfulness"]
        )
    )
    ...
    if retrieval_metrics.get(
        ...
        selected_config = retrieval_quality.get(
            "selected_config"
        )
    ...
    if dataset_depth.get(
        ...
        print(
            "OK: 20+ question target MET."
        )

    else:

        print(
            "WARNING: Less than 20 evaluation questions."
        )
    ...
    output = {
        ...
        "evaluation": (
            "comprehensive_final_evaluation"
        ),
    ...
    print(
        "Comprehensive final evaluation completed successfully."
    )
